    ...
    <div id="divstepzero">
      ...
      <h4 class="ui-bar-b">
			...
			<p>
					...
					<br />
					This tool generates a variety of basic food nutrition stock statistics for DevTreks operating budgets and capital budgets.
			</p>
			<p>
        ...
        <xsl:if test="(@AnalyzerType != 'resources01')">
          ...
          <h4 class="ui-bar-b"><strong>Step 3 of 3. Save</strong></h4>
            ...
            <p>
					  ...
					  <xsl:if test="($viewEditType = 'full')">
						  <xsl:value-of select="DisplayDevPacks:MakeDevTreksButton('saveastext', 'SubmitButton1Enabled150', 'Save As Text', $contenturipattern, $selectedFileURIPattern, 'prepaddin', 'linkedviews', 'runaddin', 'none', $calcParams4c)" />
					  </xsl:if>
				  </xsl:if>
			  </xsl:if>
			  <xsl:if test="($lastStepNumber = 'stepfour')
