...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <div>
        <a id="aFeedback" name="Feedback">
          <xsl:attribute name="href">
            mailto:<xsl:value-of select="$clubEmail" />?subject=<xsl:value-of select="$selectedFileURIPattern" />
          </xsl:attribute>
  ...
  <xsl:template match="outputgroup">
    ...
    <xsl:variable name="calculatorid">
      <xsl:value-of select="@CalculatorId"/>
    </xsl:variable>
    <xsl:choose>
  ...
  <xsl:template match="root/linkedview">
    ...
    <xsl:if test="(@TContainerSizeInSSUnits > 0)">
    ...
    <xsl:if test="(@ContainerSizeInSSUnits > 0)">
      <div data-role="collapsible" data-collapsed="true" data-theme="b" data-content-theme="d" >
        ...
        <div class="ui-grid-a">
          <div class="ui-block-a">
            Container Size : <xsl:value-of select="@ContainerSizeInSSUnits" />
          </div>
          <div class="ui-block-b">
            Serving Cost : <xsl:value-of select="@ServingCost" />
          </div>
          <div class="ui-block-a">
            USDA Servings Per Cont : <xsl:value-of select="@TypicalServingsPerContainer" />
          </div>
          <div class="ui-block-b">
            Servings Per Cont : <xsl:value-of select="@ActualServingsPerContainer" />
          </div>
          <div class="ui-block-a">
            Serving Size Unit : <xsl:value-of select="@ServingSizeUnit" />
          </div>
          <div class="ui-block-b">
            Serving Size : <xsl:value-of select="@ActualServingSize" />
          </div>
          <div class="ui-block-a">
            Water g : <xsl:value-of select="@ActualWater_g" />
          </div>
          <div class="ui-block-b">
            Energy Kcal : <xsl:value-of select="@ActualEnerg_Kcal" />
          </div>
          <div class="ui-block-a">
            Protein g : <xsl:value-of select="@ActualProtein_g" />
          </div>
          <div class="ui-block-b">
          </div>
          <div class="ui-block-a">
            Lipid g : <xsl:value-of select="@ActualLipid_Tot_g" />
          </div>
          <div class="ui-block-b">
            Ash g : <xsl:value-of select="@ActualAsh_g" />
          </div>
          <div class="ui-block-a">
            Carbohydrate g :  <xsl:value-of select="@ActualCarbohydrt_g" />
          </div>
          <div class="ui-block-b">
            Fiber (TD) g : <xsl:value-of select="@ActualFiber_TD_g" />
          </div>
          <div class="ui-block-a">
            Sugar (Tot) g : <xsl:value-of select="@ActualSugar_Tot_g" />
          </div>
          <div class="ui-block-b">
            Calcium mg : <xsl:value-of select="@ActualCalcium_mg" />
          </div>
          <div class="ui-block-a">
            Iron mg : <xsl:value-of select="@ActualIron_mg" />
          </div>
          <div class="ui-block-b">
            Magnesium mg : <xsl:value-of select="@ActualMagnesium_mg" />
          </div>
          <div class="ui-block-a">
            Phosphorus mg : <xsl:value-of select="@ActualPhosphorus_mg" />
          </div>
          <div class="ui-block-b">
            Potassium mg : <xsl:value-of select="@ActualPotassium_mg" />
          </div>
          <div class="ui-block-a">
            Sodium mg : <xsl:value-of select="@ActualSodium_mg" />
          </div>
          <div class="ui-block-b">
            Zinc mg : <xsl:value-of select="@ActualZinc_mg" />
          </div>
          <div class="ui-block-a">
            Copper mg : <xsl:value-of select="@ActualCopper_mg" />
          </div>
          <div class="ui-block-b">
            Manganese mg : <xsl:value-of select="@ActualManganese_mg" />
          </div>
          <div class="ui-block-a">
            Selenium pg : <xsl:value-of select="@ActualSelenium_pg" />
          </div>
          <div class="ui-block-b">
            Vitamin C mg : <xsl:value-of select="@ActualVit_C_mg" />
          </div>
          <div class="ui-block-a">
            Thiamin mg : <xsl:value-of select="@ActualThiamin_mg" />
          </div>
          <div class="ui-block-b">
            Riboflavin mg : <xsl:value-of select="@ActualRiboflavin_mg" />
          </div>
          <div class="ui-block-a">
            Niacin mg : <xsl:value-of select="@ActualNiacin_mg" />
          </div>
          <div class="ui-block-b">
            Panto mg : <xsl:value-of select="@ActualPanto_Acid_mg" />
          </div>
          <div class="ui-block-a">
            Vitamin B6 mg : <xsl:value-of select="@ActualVit_B6_mg" />
          </div>
          <div class="ui-block-b">
            Folate (Tot) pg : <xsl:value-of select="@ActualFolate_Tot_pg" />
          </div>
          <div class="ui-block-a">
            Folic Acid pg : <xsl:value-of select="@ActualFolic_Acid_pg" />
          </div>
          <div class="ui-block-b">
            Food Folate pg : <xsl:value-of select="@ActualFood_Folate_pg" />
          </div>
          <div class="ui-block-a">
            Folate (DFE) pg : <xsl:value-of select="@ActualFolate_DFE_pg" />
          </div>
          <div class="ui-block-b">
            Choline (Tot) mg : <xsl:value-of select="@ActualCholine_Tot_mg" />
          </div>
          <div class="ui-block-a">
            Vitamin B12 pg : <xsl:value-of select="@ActualVit_B12_pg" />
          </div>
          <div class="ui-block-b">
          </div>
          <div class="ui-block-a">
            Vitamin A (IU) : <xsl:value-of select="@ActualVit_A_IU" />
          </div>
          <div class="ui-block-b">
            Vitamin A (RAE) : <xsl:value-of select="@ActualVit_A_RAE" />
          </div>
          <div class="ui-block-a">
            Retinol pg : <xsl:value-of select="@ActualRetinol_pg" />
          </div>
          <div class="ui-block-b">
            Alpha Carotene pg : <xsl:value-of select="@ActualAlpha_Carot_pg" />
          </div>
          <div class="ui-block-a">
            Beta Carotene pg : <xsl:value-of select="@ActualBeta_Carot_pg" />
          </div>
          <div class="ui-block-b">
            Beta Crypt pg : <xsl:value-of select="@ActualBeta_Crypt_pg" />
          </div>
          <div class="ui-block-a">
            Lycopene pg : <xsl:value-of select="@ActualLycopene_pg" />
          </div>
          <div class="ui-block-b">
            Lut Zea pg : <xsl:value-of select="@ActualLut_Zea_pg" />
          </div>
          <div class="ui-block-a">
            Vitamin E mg : <xsl:value-of select="@ActualVit_E_mg" />
          </div>
          <div class="ui-block-b">
            Vitamin D pg : <xsl:value-of select="@ActualVit_D_pg" />
          </div>
          <div class="ui-block-a">
            ViVitamin D (IU) : <xsl:value-of select="@ActualViVit_D_IU" />
          </div>
          <div class="ui-block-b">
            Vitamin K pg : <xsl:value-of select="@ActualVit_K_pg" />
          </div>
          <div class="ui-block-a">
            Fatty Acid Sat g : <xsl:value-of select="@ActualFA_Sat_g" />
          </div>
          <div class="ui-block-b">
            Fatty Acid Mono g : <xsl:value-of select="@ActualFA_Mono_g" />
          </div>
          <div class="ui-block-a">
            Fatty Acid Poly g : <xsl:value-of select="@ActualFA_Poly_g" />
          </div>
          <div class="ui-block-b">
            Cholesterol mg : <xsl:value-of select="@ActualCholestrl_mg" />
          </div>
          <div class="ui-block-a">
            Extra 1 : <xsl:value-of select="@ActualExtra1" />
          </div>
          <div class="ui-block-b">
            Extra 2 : <xsl:value-of select="@ActualExtra2" />
          </div>
        </div>
        <div >
