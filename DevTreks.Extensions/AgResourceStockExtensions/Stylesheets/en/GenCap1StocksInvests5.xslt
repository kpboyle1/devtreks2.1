...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="investment">
		...
		<tr>
			<td colspan="10"><strong>Investment Totals&#xA0;&#xA0;(Date:&#xA0;<xsl:value-of select="@Date" /></strong></td>
		</tr>
		<xsl:variable name="calculatorid"><xsl:value-of select="@CalculatorId"/></xsl:variable>
		<xsl:apply-templates select="root/linkedview[@Id=$calculatorid]">
			<xsl:with-param name="localName"><xsl:value-of select="local-name()" /></xsl:with-param>
    ...
    <xsl:variable name="nodeCount" select="count(investmentinput/root/linkedview[@CalculatorType='gencapital'])"/>
    <xsl:if test="($nodeCount != '0')">
      ...
      <tr>
		...
		<xsl:if test="($nodeCount != '0') and ($viewEditType != 'summary')">
			<tr>
				<th scope="col" colspan="10">Inputs (cost = cost * component.amount * input.times * input.amount)</th>
			</tr>
      <tr>
        <td>
				  <strong>
    ...
    <xsl:if test="($nodeCount != '0')">
		  ...
		  <tr>
			  <td colspan="10"><strong>Component Totals&#xA0;&#xA0;(Amount:&#xA0;<xsl:value-of select="@Amount" />;&#xA0;Date:&#xA0;<xsl:value-of select="@Date" /></strong></td>
		  </tr>
		  <xsl:variable name="calculatorid"><xsl:value-of select="@CalculatorId"/></xsl:variable>
		  <xsl:apply-templates select="root/linkedview[@Id=$calculatorid]">
			  <xsl:with-param name="localName"><xsl:value-of select="local-name()" /></xsl:with-param>
