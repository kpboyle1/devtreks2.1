...
<!-- Author: www.devtreks.org, 2013, July -->
<xsl:stylesheet version="1.0"
    ...
    <div id="divstepzero">
      <xsl:choose>
        <xsl:when test="(contains($docToCalcNodeName, 'input')
              or contains($docToCalcNodeName, 'output')
      ...
      <h4 class="ui-bar-b"><strong>Resource Stock Analyzer Views</strong></h4>
      <h4 class="ui-bar-b">
					<strong>
			...
			<p>
					...
					<br />
					This tool generates a variety of basic resource stock statistics for DevTreks inputs.
			</p>
			<p>
	...
	<xsl:template match="linkedview">
      ...
      <div id="divsteptwo">
        ...
        <xsl:if test="(($docToCalcNodeName != 'inputseries' and $docToCalcNodeName != 'outputseries') or contains($docToCalcNodeName, 'devpack') or contains($docToCalcNodeName, 'linkedview'))">
        ...
        <div>
            ...
            <option>
						...
						<option>
							<xsl:attribute name="value">irrpower</xsl:attribute>
							<xsl:if test="(@RelatedCalculatorType = 'irrpower')">
								<xsl:attribute name="selected" />
							</xsl:if>Irrigation Power
						</option>
						<option>
							<xsl:attribute name="value">gencapital</xsl:attribute>
							<xsl:if test="(@RelatedCalculatorType = 'gencapital')">
								<xsl:attribute name="selected" />
							</xsl:if>General Capital
						</option>
				  </select>
			  </div>
        <xsl:if test="(@AnalyzerType != 'resources01')">
      ...
      <div id="divsteplast">
        ...
        <div data-role="collapsible"  data-theme="b" data-content-theme="d">
			  ...
			  <ul data-role="listview">
				  <li><strong>Step 1. Base Calculations To Analyze:</strong> The only option currently available are base input documents that include the calculator options displayed in step 2.</li>
