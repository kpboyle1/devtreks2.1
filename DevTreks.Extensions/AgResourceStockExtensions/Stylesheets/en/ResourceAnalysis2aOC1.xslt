      ...
      <xsl:choose>
         <xsl:when test="(contains($docToCalcNodeName, 'operation')
              or contains($docToCalcNodeName, 'component')
				...
				<xsl:otherwise>
					<h3>This budget analyzer does not appear appropriate for the document being analyzed. Are you 
					sure this is the right analyzer?</h3>
				</xsl:otherwise>
			</xsl:choose>
      <h4 class="ui-bar-b"><strong>Resource Stock Selection and Scheduling Analyzer Views</strong></h4>
      <h4 class="ui-bar-b">
					<strong>
			...
			<p>
					...
					<br />
					This tool generates a variety of basic resource stock selection and scheduling statistics for DevTreks operations and components. 
              The 'Penalties' tool displays full machinery costs, including timeliness penalties, for size ranges of feasible machinery combinations. 
              The feasible combinations of power and nonpower inputs must be set using the 'size range' feature of machinery input calculators.
			</p>
			<p>
	...
	<xsl:template match="linkedview">
        ...
        <div>
            ...
            <option>
						...
						<option>
							<xsl:attribute name="value">operation2</xsl:attribute>
							<xsl:if test="(@FilesToAnalyzeExtensionType = 'operation2')">
								<xsl:attribute name="selected" />
							</xsl:if>NPV Operations (Timeliness)
						</option>
						<option>
							<xsl:attribute name="value">component2</xsl:attribute>
							<xsl:if test="(@FilesToAnalyzeExtensionType = 'component2')">
								<xsl:attribute name="selected" />
							</xsl:if>NPV Components (Timeliness)
						</option>
				  </select>
			  </div>
        <div>
            ...
            <option>
						...
						<option>
							<xsl:attribute name="value">resources02a</xsl:attribute>
							<xsl:if test="(@AnalyzerType = 'resources02a')">
								<xsl:attribute name="selected" />
							</xsl:if>Resource Feasible Machinery Penalties
						</option>
				  </select>
			  </div>
      ...
      <div id="divsteptwo">
        ...
        <div>
            ...
            <option>
						...
						<xsl:if test="(@AnalyzerType = 'resources02a')">
              <option>
									<xsl:attribute name="value">agmachinery</xsl:attribute>
									<xsl:if test="(@RelatedCalculatorType = 'agmachinery')">
										<xsl:attribute name="selected" />
									</xsl:if>Agricultural Machinery
							</option>
            </xsl:if>
				  </select>
			  </div>
      ...
      <div id="divsteplast">
        ...
        <div data-role="collapsible"  data-theme="b" data-content-theme="d">
			  ...
			  <ul data-role="listview">
				  <li><strong>Step 1. Base Calculations To Analyze:</strong> Make sure the data being analyzed corresponds to either operations or components. The base calculators must have timeliness penalty calculations.</li>
