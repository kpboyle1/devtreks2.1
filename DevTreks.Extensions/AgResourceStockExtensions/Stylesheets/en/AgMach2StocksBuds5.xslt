...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="budget">
		...
		<tr>
			<td colspan="10"><strong>Budget Totals</strong></td>
		</tr>
		<xsl:variable name="calculatorid"><xsl:value-of select="@CalculatorId"/></xsl:variable>
		<xsl:apply-templates select="root/linkedview[@Id=$calculatorid]">
			<xsl:with-param name="localName"><xsl:value-of select="local-name()" /></xsl:with-param>
	...
	<xsl:template match="budgettimeperiod">
		...
		<tr>
			<td scope="row" colspan="10">
				<strong><xsl:value-of select="@EnterpriseName" />(Amount:<xsl:value-of select="@EnterpriseAmount" />Unit:<xsl:value-of select="@EnterpriseUnit" />)</strong>
			</td>
		</tr>
    ...
    <xsl:choose>
      <xsl:when test="($localName = 'budgetoperation')">
			<tr>
				<td>
    ...
    <xsl:when test="($localName = 'budgetinput')">
			<tr>
				<td>
